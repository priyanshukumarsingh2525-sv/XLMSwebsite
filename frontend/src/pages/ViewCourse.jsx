...
const ViewCourse = () => {
  ...
  useEffect(()=>{
    const handleCreator = async () =>{
      ...
      try {
        ...
        console.log("handleCreator",error)
      } 
    }
    }
    handleCreator()
  ...
  useEffect(()=>{
   ...
   try {
    ...
    const options = {
      ...
      description:"COURSE ENRONLLEMENT PAYMENT",
  ...
  const handleReview = async()=>{
    ...
    try {
      ...
      console.log("error at handleReview",error)
      toast.error(error.response.data.message)
      setRating(0)
      setComment("")
  ...
  console.log("selectedCourse",selectedCourse?.isFree)
    ...
    <div className='min-h-screen bg-gray-50 p-6'>
      <div className='max-w-6xl mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative'>
          ...
          <div className='flex-1 space-y-2 mt-5'>
                   ...
                   <div className='flex items-start flex-col justify-between'>
                    ...
                    <div className='text-yellow-500 font-medium flex gap-2'>
                      ...
                      <span className='text-gray-500'>1,200 revies</span>
                    </div>

                    <div>
                      <span className='text-lg font-semibold text-black'>
                        {selectedCourse?.isFree?" ":"$"}
                        {selectedCourse?.isFree?"FREE":selectedCourse?.price}</span>{" "}
                    ...
                    {(!isenrolled && !selectedCourse?.isFree) ? <button className='bg-black text-white px-6 py-2 rounded hover:bg-gray-700 mt-3 cursor-pointer'onClick={()=>handleEnroll(userData?._id,courseId)}>Enroll Now</button>:<button className='bg-green-100 text-green-600 px-6 py-2 rounded hover:bg-gray-700 mt-3 cursor-pointer'onClick={()=>navigate(`/viewlecture/${courseId}`)}>Watch Now</button>}

                   {/* <button className='text-red' onClick={()=>navigate(`/viewlecture/${courseId}`)}>View Lecture</button> */}
        ...
        <div>
          ...
          <p className='text-gray-700'>Beginners, aspireing developers, and professional looking to upgrade skills</p>
        </div>
        ...
        <div className='flex items-center gap-4 border-t pt-4'>
          {creatorData?.photoUrl?<img src={creatorData?.photoUrl} alt="" className='border border-gray-200 w-16 h-16 rounded-full object-cover'/>:<img src={img} alt="" className='border border-gray-200 object-cover w-16 h-16 rounded-full'/>}
        ...
        <div>
          <p className='text-xl font-semibold mb-2'>Other Publishec Courses by the Educator</p>
        </div>
