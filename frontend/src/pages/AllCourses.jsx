  ...
  return (
    <div className='flex min-h-screen bg-gray-50'>
        ...
        <aside className={`w-65 h-screen overflow-y-auto bg-black fixed top-0 left-0 p-6 py-32.5 border-r border-gray-200 shadow-md transition-transform duration-300 z-5 ${isSidebarVisible? "translate-x-0":"-translate-x-full"} md:block md:translate-x-0`}>
            <h2 className='text-xl font flex items-center justify-center gap-2 text-gray-50 mb-6'><FaArrowLeft className='text-white' onClick={()=>navigate('/')}/>Fiter by Category</h2>

            <form action="" className='space-y-4 text-sm bg-gray-600 border-white text-white border p-2.5 rounded-2xl'onSubmit={(e)=>e.preventDefault()}>
                <button className='px-2.5 py-2.5 bg-black text-white rounded-[10px] text-[15px] font-light flex items-center justify-center cursor-pointer gap-2'onClick={()=>navigate('/search')}>Search With AI <img src={ai} alt="" className='w-7.5 h-7.5 rounded-full'/></button>
        ...
        <main className=' ml-70 w-full transition-all duration-300 py-32.5 flex items-start justify-center md:justify-start flex-wrap gap-6 px-2.5'>
            {
