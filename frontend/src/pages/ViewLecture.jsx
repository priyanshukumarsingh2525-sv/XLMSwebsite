...
function ViewLecture() {
  ...
  return (
    <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
        ...
        <div className='mb-6'>
            <h2 className='text-2xl font-bold flex items-center justify-start gap-5 text-gray-800'><FaArrowLeft className='text-black w-5.5 h-5.5 cursor-pointer' onClick={()=> navigate("/")}/>{selectedCourse?.title}</h2>
        ...
        <div className='w-full md:w-1/3 bg-white rounded-2xl p-6 shadow-md border border-gray-200 h-fit'>
        ...
        <div className='flex flex-col gap-3 mb-6'>
            ...
            (selectedCourse?.lectures?.map((lecture,index)=>(
               <button key={index} onClick={()=>setSelectedLecture(lecture)} className={`flex items-center justify-between p-3 rounded-lg border transition text-left ${
                selectedCourse?._id === lecture._id?'bg-gray-200 border-gray-500':'hover:bg-gray-50 border-gray-300'
