    ...
    useEffect(()=>{
       const creatorCourses = async () =>{
        try {
            ...
            console.log("error in getCreatorCourse",error)
        }
       }
       creatorCourses()
    },[userData])
  ...
  return (
    <div className='flex min-h-screen bg-gray-100'>
      <div className='w-full min-h-screen p-4 sm:p-6 bg-gray-100'>
        <div className='flex flec-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3'>
         ...
         <div className='md:hidden space-y-4'>
            ...
            <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
                ...
                <span className={`w-fit px-3 py-1 text-xs rounded-full bg-red-100 ${course?.isPublished? "bg-green-100 text-green-600":"bg-red-100 text-red-600"}`}>{course?.isPublished?"Published":"Draft"}</span>
            </div>))}
